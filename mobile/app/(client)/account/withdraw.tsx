...
export default function WithdrawScreen() {
  ...
  async function handleWithdraw() {
    ...
    setSubmitting(true);
    try {
      await checkingAccountApi.withdraw(accountId, {
        amount,
        description: description || undefined,
      });
      Toast.show({ type: 'success', text1: 'Saque realizado' });
      router.back();
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Falha no saque';
      Toast.show({ type: 'error', text1: 'Saque', text2: message });
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <View className="px-6 pt-4">
        <View className="flex-row items-center justify-between">
          ...
          <Text className="text-lg font-semibold text-gray-900">Sacar</Text>
          <View className="w-10" />
        </View>
      </View>
      ...
      <View className="mt-6 gap-4 px-6">
        ...
        <Input
          ...
          placeholder="Ex: Saque para boleto externo"
          value={description}
